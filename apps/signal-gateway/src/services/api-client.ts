...
class APIClient {
  ...
  private get botSecret(): string {
    return process.env.SIGNAL_GATEWAY_SECRET || '';
  }
  ...
  async getChannelUser(channelUserId: string): Promise<any> {
    try {
      const response = await this.client.get(`/channels/signal/users/${channelUserId}`, {
        headers: {
  ...
  async createOrUpdateChannelUser(data: {
    ...
    const response = await this.client.post('/channels/signal/users', data, {
      headers: {
        'X-Channel-Bot-Secret': this.botSecret,
      },
    });
  ...
  async updateConversation(channelUserId: string, conversationId: string): Promise<void> {
    await this.client.post(
      `/channels/signal/users/${channelUserId}/conversation`,
      { conversationId },
      {
        headers: {
          'X-Channel-Bot-Secret': this.botSecret,
