...
export default function DevelopersBillingScreen() {
  ...
  const handlePurchaseCredits = async (packageId: string) => {
    try {
      const { url } = await createCheckoutMutation.mutateAsync({
        packageId,
        successUrl: window.location.origin + "/developers/billing?success=true",
        cancelUrl: window.location.origin + "/developers/billing",
      });
  ...
  const handleManagePayment = async () => {
    try {
      const url = await createPortalMutation.mutateAsync(window.location.origin + "/developers/billing");
      if (url) {
        await Linking.openURL(url);
      }
    } catch (error: any) {
  ...
  return (
    <ScrollView className="flex-1 bg-background">
      ...
      <View className="px-6 py-6 border-b border-border">
        ...
        <Text className="text-2xl font-semibold text-foreground">Billing & credits</Text>
        <Text className="text-sm text-muted-foreground mt-1">
          Manage your credits and API usage
        </Text>
      </View>
      ...
      {isLoading ? (
        ...
        <>
          ...
          {/* How Credits Work for Developers */}
          <View className="px-6 py-6 border-b border-border">
            <Text className="text-sm font-semibold text-foreground mb-4">For developers</Text>

            <View className="mb-3">
              <Text className="text-sm text-foreground mb-1">• API calls</Text>
              <Text className="text-sm text-muted-foreground ml-4">
                All API calls made through your developer apps consume credits from your account
              </Text>
            </View>

            <View>
              <Text className="text-sm text-foreground mb-1">• Shared balance</Text>
              <Text className="text-sm text-muted-foreground ml-4">
                Credits are shared between your personal usage and all your developer apps
              </Text>
            </View>
          </View>
