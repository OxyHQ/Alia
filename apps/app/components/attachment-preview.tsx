...
import type { Attachment } from "@/lib/globalStore";
...
function getDocumentIcon(mimeType: string, name: string) {
  ...
  if (mimeType.includes('spreadsheet') || mimeType.includes('excel') || ['xls', 'xlsx', 'csv'].includes(ext)) {
    return { Icon: FileSpreadsheet, color: '#22C55E', bgColor: '#22C55E18' };
  }
      ...
      {attachments.map((attachment) => (
        <View key={attachment.id} className="relative">
          {attachment.type === 'image' ? (
            <View className="relative rounded-2xl overflow-hidden bg-muted border border-border" style={{ width: 120, height: 120 }}>
            ...
            <View className="relative rounded-2xl border border-border overflow-hidden bg-muted/30" style={{ width: 180, height: 80 }}>
              <View className="flex-1 flex-row items-center px-3 gap-3">
                {/* Type-specific colored icon */}
                <View
                  className="w-10 h-10 rounded-xl items-center justify-center"
                  style={{ backgroundColor: getDocumentIcon(attachment.mimeType, attachment.name).bgColor }}
                >
                  {(() => {
                    const { Icon, color } = getDocumentIcon(attachment.mimeType, attachment.name);
