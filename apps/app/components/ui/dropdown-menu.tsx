...
function DropdownMenuContent({
  ...
  // Use Fragment instead of FullWindowOverlay when inside modal (portalHost="fullscreen-modal")
  const OverlayComponent = portalHost === 'fullscreen-modal' ? React.Fragment : FullWindowOverlay;
  ...
  return (
    <TextClassContext.Provider
      ...
      <DropdownMenuPrimitive.Item
        className={cn(
          'active:bg-accent group relative flex flex-row items-center gap-2 rounded-lg px-2.5 py-1.5',
          Platform.select({
            web: cn(
  ...
  return (
    <TextClassContext.Provider value="text-sm text-popover-foreground select-none group-active:text-accent-foreground">
      <DropdownMenuPrimitive.RadioItem
        className={cn(
          'active:bg-accent group relative flex flex-row items-start gap-2.5 rounded-lg py-1.5 pl-7 pr-2.5',
          Platform.select({
            web: 'focus:bg-accent focus:text-accent-foreground cursor-default outline-none data-[disabled]:pointer-events-none',
          }),
          props.disabled && 'opacity-50',
        ...
        {...props}>
        <View className="absolute left-2 top-2 flex h-4 w-4 items-center justify-center">
          <DropdownMenuPrimitive.ItemIndicator>
            <Icon as={Check} className="text-foreground size-3.5" />
          </DropdownMenuPrimitive.ItemIndicator>
        </View>
        <>{children}</>
      </DropdownMenuPrimitive.RadioItem>
