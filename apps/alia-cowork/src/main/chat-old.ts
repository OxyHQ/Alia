    ...
    try {
      ...
      const result = streamText({
        ...
        onFinish: async (event) => {
          ...
          // Capture final usage
          if (event.usage) {
            totalPromptTokens = event.usage.promptTokens || 0
            totalCompletionTokens = event.usage.completionTokens || 0
      ...
      for await (const chunk of result.fullStream) {
        ...
        if (chunk.type === 'text-delta' && 'text' in chunk && chunk.text) {
          // Extract thinking tags for chain of thought visualization (Anthropic, DeepSeek, etc.)
        ...
        } else if (chunk.type === 'reasoning-delta' || (chunk as any).type === 'thought-delta') {
          ...
          if (reasoningText && typeof reasoningText === 'string' && reasoningText.trim()) {
            console.log('[ChatProvider] Reasoning chunk:', reasoningText.slice(0, 100))
            this.send('chat:thinking', { content: reasoningText.trim() })
          }
