/**
 * Chat Provider using AI SDK 6 with Custom Provider
 * Streams responses using AI SDK streamText() with Alia custom provider
 */
  ...
  async handleMessage(
    ...
    try {
      ...
      const result = streamText({
        model: alia.languageModel(selectedModel),
        ...
        onChunk: async ({ chunk }) => {
          if (chunk.type === 'text-delta') {
            ...
            console.log('[ChatProvider] Tool call:', chunk.toolName, (chunk as any).input)

            // Handle set_mode specially
            if (chunk.toolName === 'set_mode') {
              this.currentMode = ((chunk as any).input as any).mode
            ...
            const success = !String((chunk as any).output).includes('Error:')
            this.send('chat:toolResult', {
              tool: chunk.toolName,
              success,
              result: String((chunk as any).output).slice(0, 500)
            })
          }
        },
        onError: (event) => {
