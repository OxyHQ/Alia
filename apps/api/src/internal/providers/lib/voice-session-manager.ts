...
import { resolveClarityModel } from './model-resolver.js';
import {
  reserveVoiceCredits,
...
export class VoiceSessionManager {
  ...
  async createSession(
    ...
    // Resolve Clarity model to provider model
    const resolved = await resolveClarityModel(model, 1000);
    if (!resolved) {
      throw new Error(`Unable to resolve model: ${model}`);
    }

    const { provider, modelId, keyConfig, clarityModel } = resolved;
    ...
    // Get cost per minute
    const { getModelMappingsForTier } = await import('./clarity-models.js');
    const mappings = getModelMappingsForTier(clarityModel.tier);
    ...
    try {
      ...
      const t2 = Date.now();
      const agentBridge = new LiveKitAgentBridge('clarity-agent');
      const agentToken = await createAgentToken(roomName, 'clarity-agent');
  ...
  async closeSession(sessionId: string, reason?: string): Promise<void> {
    ...
    try {
      ...
      if (session.creditReservation) {
        try {
          await finalizeVoiceCredits(session.creditReservation, actualMinutes, session.clarityModelId, session.costPerMinute);
      ...
      if (session.cohostCreditReservation) {
        try {
          await finalizeVoiceCredits(session.cohostCreditReservation, session.cohostMinutesElapsed || 0.01, session.clarityModelId, session.cohostCostPerMinute);
  ...
  async enableCohost(sessionId: string): Promise<void> {
    ...
    try {
      ...
      const resolved = await resolveClarityModel(session.clarityModelId, 1000);
      ...
      const cohostReservation = await reserveVoiceCredits(
        session.userId, 1, session.clarityModelId, session.costPerMinute
      );
      ...
      const cohostBridge = new LiveKitAgentBridge('clarity-cohost');
      const cohostToken = await createAgentToken(session.roomName, 'clarity-cohost');
      ...
      if (lastPrimary) {
        ...
        this.injectTranscriptAndTrigger(session, 'cohost', `[Clarity] ${lastPrimary.text}`);
      }
  ...
  private handleCohostTurnComplete(session: VoiceSession, completedRole: 'primary' | 'cohost'): void {
    ...
    const prefix = completedRole === 'primary' ? '[Clarity]' : '[Cohost]';
  ...
  private buildCohostInstructions(session: VoiceSession): string {
    ...
    return `You are "Cohost", a second voice in a live audio conversation with Clarity (the main assistant) and a human user. You can hear everything — the user's voice and Clarity's voice come through your audio input.
...
- Don't repeat what Clarity just said. Don't summarize. Add something new or stay quiet.
...
- If the conversation is flowing well between Clarity and the user, it's fine to just listen for several turns.
...
${base}`;
  ...
  private async injectContextIntoCohost(session: VoiceSession): Promise<void> {
    ...
    for (const entry of session.recentTranscripts) {
      const prefix = entry.speaker === 'primary' ? '[Clarity]' : entry.speaker === 'user' ? '[User]' : '[Cohost]';
      session.cohostProviderSocket.send(JSON.stringify({
        type: 'conversation.item.create',
        item: {
          type: 'message',
  ...
  private startBillingTimer(session: VoiceSession): void {
    ...
    session.billingTimer = setInterval(async () => {
      ...
      try {
        ...
        const creditsForNextMinute = await calculateCreditsFromMinutes(1, session.clarityModelId, session.costPerMinute);
  ...
  private async saveUsageRecord(session: VoiceSession, isFinal: boolean, disconnectReason?: string): Promise<void> {
    try {
      ...
      const record: any = {
        ...
        clarityModelId: session.clarityModelId,
