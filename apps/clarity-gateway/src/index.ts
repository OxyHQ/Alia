...
const allowedOrigins = process.env.CORS_ORIGINS
  ...
  : ['https://gateway.clarity.oxy.so', 'http://localhost:5173', 'http://localhost:3001'];
...
async function start() {
  ...
  const server = app.listen(PORT, () => {
    log.general.info('clarity-gateway listening on port %d', PORT);
  });
...
start().catch((err) => {
  log.general.fatal({ err }, 'Failed to start clarity-gateway');
  process.exit(1);
});
