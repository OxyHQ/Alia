...
// Auth health stats -- stubbed for standalone service
async function getAuthHealthStats(_hours: number): Promise<any[]> {
  return [];
}
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const allHealthy = stats.length === 0 || stats.every((s: any) => s.isHealthy);
