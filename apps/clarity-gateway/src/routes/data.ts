 ...
 * GET /api/models   — Clarity models, tier mappings, availability
...
/**
 * GET /api/models
 * Returns Clarity models with optional tier mappings and availability.
 * Query params:
...
router.get('/models', async (req: Request, res: Response) => {
  try {
    ...
    if (id && typeof id === 'string') {
      const model = getClarityModel(id);
      return res.json({
        success: true,
        data: { model, isClarityModel: isClarityModel(id) },
      });
    }

    // Tier mappings for a specific tier
    if (tier && typeof tier === 'string') {
      const mappings = getModelMappingsForTier(tier as ClarityTier);
      return res.json({
        success: true,
        data: { tier, mappings },
      });
