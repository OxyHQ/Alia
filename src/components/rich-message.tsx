                ...
                <div className="space-y-2 w-full">
                    {banners.map((banner, index) => (
                        <div key={`banner-${index}`} className={cn(
                            ...
                            <p className="text-sm opacity-90 whitespace-pre-wrap">{banner.content}</p>
                        </div>
                    ))}
                </div>
            )}
                        ...
                        <div key={`comparison-${index}`} className="rounded-lg border bg-card text-card-foreground shadow-sm overflow-hidden w-full">
                            ...
                            <div className="grid md:grid-cols-2 gap-0 divide-y md:divide-y-0 md:divide-x">
                                <div className={cn("p-4 min-w-0", comp.left.tone === 'danger' ? 'bg-red-50/30' : 'bg-muted/10')}>
                                    ...
                                    <p className="text-sm text-muted-foreground whitespace-pre-wrap">{comp.left.content}</p>
                                </div>
                                <div className={cn("p-4 min-w-0", comp.right.tone === 'success' ? 'bg-green-50/30' : 'bg-muted/10')}>
                                    ...
                                    <p className="text-sm text-muted-foreground whitespace-pre-wrap">{comp.right.content}</p>
                                </div>
                            </div>
                            {comp.conclusion && (
                                    ...
                                    {timeline.events.map((event, eventIndex) => (
                                        <div key={eventIndex} className="relative pl-6">
                                            ...
                                            <div className="flex flex-col min-w-0 gap-1">
                                                ...
                                                <span className="font-medium text-sm leading-tight break-words">{event.title}</span>
                                                {event.description && <p className="text-xs text-muted-foreground whitespace-pre-wrap leading-normal">{event.description}</p>}
                                            </div>
                                        </div>
                                    ))}
                                </div>
