...
export async function callDigitalOceanAsyncInvoke(
  ...
  log.providers.debug({ modelId, request_id }, 'DO async-invoke submitted');
  ...
  while (Date.now() < deadline) {
    ...
    if (!statusRes.ok) {
      log.providers.warn({ request_id, status: statusRes.status, pollCount }, 'DO async-invoke status poll failed');
    ...
    if (statusData.status === 'COMPLETED') {
      log.providers.debug({ request_id, pollCount }, 'DO async-invoke completed');
      break;
    }
