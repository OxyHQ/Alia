...
vi.mock('@/features/chat/ui/chat-workspace', async () => {
  const ReactModule = await import('react');
  return {
    ChatWorkspace: ({
      background,
...
vi.mock('@/features/chat/ui/chat-interface', () => ({ ChatInterface: () => null }));
...
// The draft lives in the per-conversation draft store, outside the screen: a
// module-level value stands in for it, so it outlives the composer's unmount.
const draft = vi.hoisted(() => ({ text: '' }));
vi.mock('@/features/chat/ui/composer/use-alia-composer', () => ({
  useAliaComposer: () => ({
    ...
    attachments: [],
    address: { account: 'account-1', target: null },
    turnOptions: {},
    restoreTurn: vi.fn(),
    clearTurn: vi.fn(),
    props: {},
...
vi.mock('@/features/chat/ui/composer/composer-suggestions', () => ({
...
vi.mock('@/features/chat/runtime/use-turn-edit', () => ({
...
vi.mock('@/features/chat/runtime/global-store', () => {
  const state = { setGhostMode: () => undefined };
  const useStore = (selector: (s: typeof state) => unknown) => selector(state);
  useStore.getState = () => state;
  return { useStore };
});
vi.mock('@/features/projects/runtime/projects-store', () => ({
