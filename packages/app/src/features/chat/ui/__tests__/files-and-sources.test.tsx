...
import { FilesAndSources } from '@/features/chat/ui/execution/files-and-sources';
...
const LONG_NAME =
  'a-very-long-generated-report-name-that-does-not-fit-in-three-hundred-pixels-2026-09-10.md';
...
const labelled = (r: ReactTestRenderer, label: string) =>
  r.root.findAll(
    (node) =>
      ...
      (node.type === HOST_PRESSABLE || node.type === HOST_VIEW),
  );
...
describe('with outputs and sources', () => {
  it('lists each output by its whole name, as a control only when it can be opened', () => {
    ...
    const openable = buttons(r).find(
      (b) => b.props.accessibilityLabel === 'thought.openOutput(name=qa-ui.md)',
    );
    ...
    const nameText = r.root.findAll(
      (node) => node.type === HOST_TEXT && node.props.children === LONG_NAME,
    );
  ...
  it('folds a section on its header and unfolds it again', () => {
    ...
    act(() => {
      buttons(r)
        .find((b) => b.props.accessibilityLabel === 'thought.outputs')!
        .props.onPress();
    });
...
describe('with nothing', () => {
  it('keeps both sections, each saying it is empty, and ships no add buttons', () => {
    ...
    expect(buttons(r).map((b) => b.props.accessibilityLabel)).toEqual([
      'thought.outputs',
      'thought.sources',
    ]);
  });
