...
function code(file: string): string {
  return readFileSync(path.join(UI, file), 'utf8')
    .split('\n')
    .filter((l) => !/^\s*(\/\/|\*|\/\*)/.test(l))
    .join('\n');
}
...
describe('the body family is derived from Bloom, not written out', () => {
  ...
  it('reads Bloom’s own sans token', () => {
    expect(markdown).toMatch(/from ['"]@oxy\.so\/bloom\/fonts['"]/);
...
describe('every AliaMarkdown call site hands the family over', () => {
  ...
  const CALL_SITES = ['markdown.tsx'] as const;
