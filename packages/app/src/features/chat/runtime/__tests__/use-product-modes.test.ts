...
const { parseModes, modeForProfile, modeById, presentation } = await import(
  "@/features/chat/runtime/use-product-modes"
);
type CatalogueEntry = import("@/features/chat/runtime/use-catalogue").CatalogueEntry;
...
describe("the picker consumes the endpoint (a call, not a mention)", () => {
  ...
  const selector = readFileSync(
    fileURLToPath(new URL("../../ui/composer/model-lineup.ts", import.meta.url)),
    "utf8",
  );
