...
vi.mock('@/lib/constants/capability-families', () => ({ capabilityIconForTool: () => undefined }));
...
vi.mock('@oxy.so/bloom/agent-progress', async () => ({ AgentProgress: (await import('./panel-bloom-stubs')).host('AgentProgress') }));
vi.mock('@oxy.so/bloom/agent-thinking', async () => ({ AgentThinking: (await import('./panel-bloom-stubs')).host('AgentThinking') }));
vi.mock('@oxy.so/bloom/ai-chat', () => ({ useAiChatShell: () => ({ compact: false }) }));
vi.mock('@oxy.so/bloom/button', async () => ({ Button: (await import('./panel-bloom-stubs')).host('Button') }));
vi.mock('@oxy.so/bloom/card', async () => ({ Card: (await import('./panel-bloom-stubs')).host('Card') }));
vi.mock('@oxy.so/bloom/link-preview', async () => ({ LinkPreviewCard: (await import('./panel-bloom-stubs')).host('LinkPreviewCard') }));
vi.mock('@oxy.so/bloom/notification', async () => ({ Notification: (await import('./panel-bloom-stubs')).host('Notification') }));
vi.mock('@oxy.so/bloom/stat-bar', async () => ({ StatBar: (await import('./panel-bloom-stubs')).host('StatBar') }));
...
vi.mock('@oxy.so/bloom/icons/RiComputerLine', async () => (await import('./panel-bloom-stubs')).iconModule('RiComputerLine'));
vi.mock('@oxy.so/bloom/icons/RiErrorWarningLine', async () => (await import('./panel-bloom-stubs')).iconModule('RiErrorWarningLine'));
vi.mock('@oxy.so/bloom/icons/RiFileTextLine', async () => (await import('./panel-bloom-stubs')).iconModule('RiFileTextLine'));
vi.mock('@oxy.so/bloom/icons/RiFolderOpenLine', async () => (await import('./panel-bloom-stubs')).iconModule('RiFolderOpenLine'));
vi.mock('@oxy.so/bloom/icons/RiSearchLine', async () => (await import('./panel-bloom-stubs')).iconModule('RiSearchLine'));

import { AgentPanel, agentStepLabel } from '@/components/agent-panel';
import type { AgentActivityEvent } from '@/lib/hooks/use-agent-activity';
