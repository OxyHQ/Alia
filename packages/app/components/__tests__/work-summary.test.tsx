...
vi.mock('@/components/lottie-loader', async () => {
  const ReactModule = await import('react');
  return {
    LottieLoader: (props: Record<string, unknown>) =>
      ReactModule.createElement('LottieLoader', props),
  };
});
...
vi.mock('@/lib/hooks/use-translation', () => ({
  useTranslation: () => ({
    t: (key: string, params?: Record<string, unknown>) => {
      const leaf = key.startsWith('thought.')
        ? (en.thought as Record<string, string>)[key.slice('thought.'.length)]
        : undefined;
...
const finished = (id: string): ToolInvocation => ({
  ...
  args: { query: 'anything' },
  result: {
    results: [{ title: 'R', url: `https://${id}.test/`, snippet: 's' }],
    count: 1,
  },
});
const pending = (id: string): ToolInvocation => ({
  toolCallId: id,
  toolName: 'webSearch',
  state: 'call',
  args: { query: 'q' },
...
const summary = (
  partial: Partial<React.ComponentProps<typeof WorkSummary>> = {},
) => (
  <WorkSummary
    messageId="a1"
...
describe('a completed turn', () => {
  it('reads "Worked for 10s", collapsed, and lists its calls when opened', () => {
    ...
    press(toggle);
    expect(summaryButton(r).props.accessibilityState).toMatchObject({
      expanded: true,
    });
  ...
  it('opens a call into its input and output, and offers the panel from there', () => {
    ...
    const row = buttons(r).find(
      (b) => b.props.accessibilityLabel === 'webSearch',
    );
    ...
    const details = buttons(r).find(
      (b) => b.props.accessibilityLabel === 'View details',
    );
...
describe('a running turn', () => {
  it('reads "Working", opens by itself, ticks, and spins the call that has not returned', () => {
    const r = render(
      summary({
        lifecycle: 'running',
        invocations: [finished('t1'), pending('t2')],
        startedAt: NOW - 5_000,
        endedAt: null,
      }),
    );
...
describe('a turn that did not finish', () => {
  ...
  it('reads "Stopped after", and its unreturned call as stopped rather than spinning', () => {
    ...
    const rows = buttons(r).filter((b) =>
      String(b.props.accessibilityLabel).startsWith('webSearch'),
    );
    expect(rows.map((b) => b.props.accessibilityLabel)).toEqual([
      'webSearch',
      'webSearch, Stopped',
    ]);
  });
  ...
  it('reads a call whose result is an error as failed', () => {
    ...
    expect(
      buttons(r).some(
        (b) => b.props.accessibilityLabel === 'webSearch, Failed',
      ),
    ).toBe(true);
  });
});
...
describe('the helpers the row reads', () => {
  it('brackets a persisted turn between the send and the save, and treats a same-instant pair as unknown', () => {
    ...
    expect(turnTiming(messages[1], messages)).toEqual({
      startedAt: Date.parse(messages[0].createdAt!),
      endedAt: Date.parse(messages[1].createdAt!),
    });
    // The placeholder a client just appended: both stamps are the send.
    expect(turnTiming(messages[3], messages)).toEqual({
      startedAt: Date.parse(messages[2].createdAt!),
      endedAt: null,
    });
    // No stamps at all.
    expect(turnTiming(messages[4], messages)).toEqual({
      startedAt: Date.parse(messages[2].createdAt!),
      endedAt: null,
    });
    expect(turnTiming({ id: 'x' }, [])).toEqual({
      startedAt: null,
      endedAt: null,
    });
  });
  ...
  it("reads a call's status off its state and the turn's liveness", () => {
    ...
    expect(
      toolCallStatus({ state: 'result', result: { error: 'x' } }, false),
    ).toBe('error');
    expect(
      toolCallStatus({ state: 'result', result: 'plain text' }, false),
    ).toBe('done');
  });

  it('lists the files a turn generated, and nothing for a generation that never returned', () => {
    const generated: ToolInvocation = {
      toolCallId: 'g1',
      toolName: 'generateFile',
      state: 'result',
      result: { filename: 'qa-ui.md', content: '#' },
    };
    const artifact: ToolInvocation = {
      toolCallId: 'g2',
      toolName: 'someTool',
      state: 'result',
      result: { artifact: { title: 'Chart' } },
    };
    const unfinished: ToolInvocation = {
      toolCallId: 'g3',
      toolName: 'generateFile',
      state: 'call',
      args: { filename: 'late.md' },
    };
    expect(
      extractOutputs([generated, artifact, unfinished, finished('t1')]),
    ).toEqual([
