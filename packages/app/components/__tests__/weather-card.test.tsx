...
vi.mock('@/lib/useColorScheme', () => ({
  useColorScheme: () => ({ colors: { primary: '#d269e6' } }),
}));
...
function render(data: WeatherCardData) {
  ...
  act(() => {
    tree = create(<WeatherCard data={data} />);
  });
  return tree!;
}
...
const dayButtons = (tree: ReactTestRenderer) =>
  tree.root.findAll(
    (n) =>
      n.props.accessibilityState !== undefined && typeof n.type !== 'string',
  );
...
describe('WeatherCard', () => {
  ...
  it('converts to Fahrenheit without asking anyone', () => {
    ...
    const toggle = tree.root.findByProps({
      accessibilityLabel: 'weather.toggleUnit',
    });
    act(() => {
  ...
  it('opens on today and moves the selection when another day is pressed', () => {
    ...
    act(() => {
      dayButtons(tree)[1].props.onPress();
    });
  ...
  it('charts the hours of the day you selected, not the whole week', () => {
    ...
    const pathOf = (t: ReactTestRenderer) =>
      t.root.findAll(
        (n) => typeof n.props.d === 'string' && n.props.fill === 'none',
      )[0].props.d;
