...
export const AITextNode = memo(function AITextNode({ id, data, selected }: AITextNodeProps) {
  ...
  /**
   * The model this node asks for, "Default (<name>)" when it names none, or
   * nothing while the catalogue has not loaded and the node names no model.
   */
  const modelLabel = labelForNode(data.model, catalogue);
