/**
 * Alia's product chat stream, as the CLI consumes it (`POST /alia/chat`).
 *
 * ## Why this replaces the `openai` package here
 *
 * `streamChat` used to go through `new OpenAI({ baseURL: `${baseUrl}/v1` })`,
 * which derives `POST /v1/chat/completions` itself. The handler is the same one
 ...
 * event, and had no notion of `alia_meta.synthetic`: the stand-in the server
 * streams when every provider is busy — "I'm sorry, all models are currently
 * busy" — was printed as the answer, pushed into the conversation as an
 * assistant turn and SAVED into the session file, so `codea resume` replayed
 * it into the next request as something the assistant had said. The
 * `docs/migration/inventories/product-api.json` row for this file records the
 * same thing from the other side: "because it ignores named events, it is the
 * client most likely to break on a change to the DEFAULT data: frame".
 *
 * There was also no way to cancel: Ctrl+C stopped the rendering while the
 * request ran — and was billed — to completion. The stream takes a signal now.
 *
 ...
 *    a handful the CLI has no use for (`alia.title`, `alia.agent`,
 ...
 *
 * `packages/alia-cowork` and `packages/alia-codea` carry the same reader, for
 * the reason `./catalogue.ts` states: this package is PUBLISHED and cannot
 * depend on an unpublished workspace package. The three are kept in step by
 * their tests, which feed the same frames.
 */
