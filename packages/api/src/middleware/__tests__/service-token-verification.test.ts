...
describe("no token is read without being verified (#139 ws15)", () => {
  ...
  it("neither server package imports a JWT library", () => {
    ...
    expect(
      SERVER_SOURCES.some((f) => f.includes("/packages/integrations/")),
    ).toBe(true);
...
describe("every principal a request can acquire is mapped (#139 ws15)", () => {
  ...
  it("no header-derived grant happens before its constant-time compare", () => {
    ...
    for (const grant of HEADER_GRANTS) {
      ...
      expect(
        anchor,
        `${grant.file}: anchor "${grant.after}" is gone`,
      ).toBeGreaterThan(-1);

      const guardAt = source.indexOf(grant.guard, anchor);
      expect(guardAt, `${grant.file}: ${grant.guard} is gone`).toBeGreaterThan(
        -1,
      );
      ...
      expect(
        GRANT.test(before),
        `${grant.file}: granted before ${grant.guard}`,
      ).toBe(false);
      ...
      expect(
        GRANT.test(after),
        `${grant.file}: nothing is granted after ${grant.guard}`,
      ).toBe(true);
    }
    ...
    const mutated = code(
      path.join(REPO_ROOT, "packages/api/src/middleware/auth.ts"),
    ).replace(/crypto\.timingSafeEqual/g, "looseEquals");
...
describe("inbound verification is @oxy.so/core, not a local implementation (#139 ws15)", () => {
  it("the API verifies through the SDK middleware and mounts the service one", () => {
    ...
    const internal = code(
      path.join(REPO_ROOT, "packages/api/src/routes/internal.ts"),
    );
  ...
  it("the integrations service compares its shared secret with the SDK helper", () => {
    ...
    // account operation.
    const index = code(
      path.join(REPO_ROOT, "packages/integrations/src/index.ts"),
    );
  ...
  it("the outbound Kaana hop presents a token and verifies none", () => {
    ...
    const modules = execFileSync(
      "git",
      ["ls-files", "--", "packages/api/src/lib/inference"],
      {
        cwd: REPO_ROOT,
        encoding: "utf8",
      },
