...
describe('sanitizeMessage', () => {
  it('strips known provider names', () => {
    expect(sanitizeMessage('OpenAI returned a 429 error')).toBe('Alia returned a 429 error');
    expect(sanitizeMessage('Anthropic rate limit exceeded')).toBe('Alia rate limit exceeded');
    expect(sanitizeMessage('Google API quota exhausted')).toBe('Alia API quota exhausted');
  });

  it('strips provider names case-insensitively', () => {
    expect(sanitizeMessage('OPENAI error')).toBe('Alia error');
    expect(sanitizeMessage('groQ timeout')).toBe('Alia timeout');
  });

  it('strips model identifiers', () => {
    // gpt- prefix is in PROVIDER_PATTERNS, so "gpt-4o-mini" first becomes "Alia4o-mini"
  ...
  it('strips multiple providers in one message', () => {
    ...
    expect(result).toBe('Tried Alia then Alia, both failed');
  });
});
...
describe('getSafeErrorMessage', () => {
  it('returns sanitized message from Error objects', () => {
    ...
    expect(getSafeErrorMessage(error, 'fallback')).toBe('Alia API returned 500');
  });
  ...
  it('sanitizes the fallback too', () => {
    expect(getSafeErrorMessage(null, 'OpenAI failed')).toBe('Alia failed');
  });
});
...
describe('sanitizeFull', () => {
  it('strips both provider names and secrets', () => {
    ...
    expect(result).toBe('Alia key [REDACTED] leaked');
  });

  it('handles messages with only provider names', () => {
    expect(sanitizeFull('Anthropic error')).toBe('Alia error');
  });
