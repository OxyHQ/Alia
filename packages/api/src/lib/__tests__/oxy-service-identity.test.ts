...
async function loadHydration(
  ...
  vi.stubEnv('ALIA_KAANA_CREDENTIAL_KEY', credential === 'configured' ? API_KEY : undefined);
  vi.stubEnv('ALIA_KAANA_CREDENTIAL_SECRET', credential === 'configured' ? API_SECRET : undefined);
...
describe('an Oxy account is hydrated as Alia, not as nobody', () => {
  ...
  it('says so in the log rather than only in the blank name', async () => {
    ...
    expect(logged.warn).toHaveBeenCalledWith(
      { unset: ['ALIA_KAANA_CREDENTIAL_KEY', 'ALIA_KAANA_CREDENTIAL_SECRET'] },
