 ...
 * answer to a question that has one. What Alia adds is exactly two things: the
...
export function getCrowdSourceClient(): CrowdSource | undefined {
  ...
  if (!serviceKey) {
    ...
    log.general.error({ reason: configurationError }, '[CrowdSource] enabled but not configured');
    return undefined;
  }
  ...
  try {
    ...
    log.general.error({ reason: configurationError }, '[CrowdSource] service key rejected');
    return undefined;
  }
}
