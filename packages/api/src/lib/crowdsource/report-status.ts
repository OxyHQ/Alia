import { ReportStatus, type ModerationLocalStatus } from '../../value-sets/report.js';
 ...
 * produce them:
 *
 * - `violation` → `resolved`: the case reached a conclusion Alia can act on.
...
/**
 * Decision statuses that end Alia's side of the case.
 *
 * A `provisional` decision leaves the report at `submitted`: §9.6 allows a later
 * revision to supersede it, and a report Alia had already closed would have to be
