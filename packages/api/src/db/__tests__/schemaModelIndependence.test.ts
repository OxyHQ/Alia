 ...
 *
 * The tuples now live in `src/value-sets/`, read by BOTH stores — the model's
 ...
 * `value-sets` module importing ANYTHING is the subtler one: the whole point of
...
describe('the Postgres schema does not depend on the Mongoose models', () => {
  ...
  const valueSetFiles = trackedSources('src/value-sets');
  ...
  it('scans a non-empty set of files in both directories', () => {
    ...
    expect(valueSetFiles.length).toBeGreaterThanOrEqual(20);
  });
  ...
  it('every value-set module is a leaf — it imports nothing', () => {
    const offenders = valueSetFiles.flatMap(({ file, text }) =>
