...
-- `pre` because every column is additive: `credit_renews` carries a default so
-- existing rows satisfy the new CHECK, and the image still serving reads none of
-- them. The marker is REQUIRED, not a default — the migrator refuses a file that
-- does not declare its side, which is how this file first failed.
