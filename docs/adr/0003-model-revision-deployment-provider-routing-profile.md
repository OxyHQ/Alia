...
- `route:instant` is a routing policy. `packages/api/src/internal/providers/lib/generate-model-mappings.ts:36` maps that single identifier onto four distinct models from four different publishers, ranked by price and quality. Which one answers a given request depends on health and cost at that instant.
- `route:code` (`packages/api/src/internal/providers/lib/alia-models.ts:89`) and `route:thinking` (`:174`) encode a surface preset and a reasoning setting as model identities, even where the underlying weights are unchanged.
...
**Example:** the product modes Auto, Instant, Thinking, Pro, Research and Code. Each names a policy over the catalogue, not a set of weights.
