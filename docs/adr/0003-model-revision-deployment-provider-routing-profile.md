...
- **Fallback policy is honoured.** End-to-end tests for `no fallback` with an unavailable concrete model, and for same-model deployment fallback, are *not yet enforced — tracked by #139 workstream 19*.
