...
- **Entitlement checks and cost-centre attribution.** Contract tests over the entitlement API and over cost-centre attribution per surface are *not yet enforced — tracked by #139 workstream 19*.
- **`cost_entries.cost_usd` is never a billing source.** Enforced today only by the schema comment at `packages/api/src/db/schema/usage.ts:24` and by code review. A check failing when a customer-charging path reads that column is *not yet enforced — tracked by #139 workstream 19*.
- **Historical receipts are immutable.** Enforcement belongs to the Oxy ledger, not to this repository. Alia's obligation is narrower and reviewable here: no Alia code path writes to a financial record, so no Alia code path can rewrite one.
