# Relay client gap analysis

The pre-publish half of [issue #139](https://github.com/OxyHQ/Alia/issues/139)
workstream 3, *"Introduce a typed Relay client boundary"*.

For every capability that workstream lists under **Client responsibilities** and
**Resilience**, this document states three things: what the Oxy↔Relay contract
...
`epic139/ws3-relay-seam`, based on `main` at `bd7d281e`. Contract paths are
...
`Oxy API → Relay` does not exist in any form. Any Alia work that assumes it does
...
- Oxy's `packages/api/src/server.ts:647` mounts `/v1` on `aliaRoutes` with only
  ...
  **zero** match `inference` or `relay` (positive control: 155 match `a`, so the
  listing is reading files). The one occurrence of "relay" in `server.ts` is at
  line 673 and refers to the E2E-encrypted device-transfer relay — an identity
...
`Alia backend → Relay`; the only live coupling is `Oxy → Alia`, and Alia is the
data plane in it.
...
- Do not build a Relay HTTP client against a URL. There is nothing to point it
...
delegated `userId` and nothing else. The Relay client attaches the principal from
...
emits OpenAI frames, sitting *above* the Relay client. Set
...
inspects provider error shapes, which is Relay's job once Relay reports a typed
...
**Edit.** `AliaModelChoice` (§4) crosses the seam and the Relay client turns it
into a `RoutingTarget`. The re-resolve loop at `provider-loop.ts:134-154` is
**deleted**, not ported: choosing a different provider is Relay's, and keeping a
...
- **`maxRetries: 0`** (`model-config.ts:63`) deliberately disables the AI SDK's
  ...
  deleted (§3.10), retry policy is entirely Relay's and this line has nowhere to
...
### 4.5 Circuit behaviour for Relay unavailability at the product boundary
...
(workstream 7); a Relay-level breaker is built fresh at the seam.
...
**Edit.** Delete the branch and the else-arm together. Until the Relay client can
...
accounting (Relay), and any wiring. A half-wired seam is worse than an unwired
...
- **§1 expires** if `packages/api/src/routes/` in OxyHQ/oxy gains a file matching
  `inference` or `relay`, or if `server.ts` mounts one. The check is one
  `git ls-tree`.
