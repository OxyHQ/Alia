...
system.** All 97 `BLOCKED_KAANA` rows were re-classified, because the premise under them — that the
Kaana data plane was not deployed — had stopped being true. Seven survive that verdict, each naming
a specific missing capability. See [the re-audit](#the-kaana-re-audit-2026-08-19) and
...
**The table below is the 2026-08-19 re-audit's.** `BLOCKED_KAANA` fell from 97 to 7 — not because
work landed in this repository, but because the premise under it stopped being true. Kaana is
deployed and serving. See [the re-audit](#the-kaana-re-audit-2026-08-19).

| verdict | all 246 | still open | meaning |
| --- | ---: | ---: | --- |
| `BLOCKED_CUTOVER` | 58 | 58 | the destination exists, is deployed and was measured; it waits on Alia actually routing through Kaana — every step in [the cutover gap](#the-cutover-what-it-would-take-in-production-today) |
...
| `ACTIONABLE_UPSTREAM` | 17 | 17 | doable today, but the deliverable is in `OxyHQ/Kaana` or `OxyHQ/oxy`, not here |
...
| `BLOCKED_KAANA` | 7 | 7 | needs a **named** Kaana capability absent at contract 1.1.0 — non-text modalities, per-customer failover authorisation, or a quota-header mapping |
| `ROLLUP_OF` | 1 | 1 | a conjunction of other boxes; true only when every component is, so it is ticked last |

At the 2026-08-17 audit the same 246 rows read `BLOCKED_KAANA` 94, `ACTIONABLE_NOW` 42,
...
| `textCannotSeeAnInvertedCondition` | **A source-text assertion cannot see a negation.** Naming a function is not acting on its answer | `toContain('kaanaBlocksReadiness()')` passes with `if (!kaanaBlocksReadiness())`, which reports not-ready exactly when Kaana *is* reachable — **1387 tests passed**. L394 was un-ticked |
...
| `liveIsNotServable` *(re-audit)* | **A green probe and a fresh artefact prove the process is up and the credential valid — never that the thing behind them will answer.** Ask which *endpoint* the freshness signal exercised | Kaana is 2/2, `/livez` is 200, and its publisher re-issued the inventory 14 min before it was read — while its only provider account answered **402 `payment_required`** to every chat completion, because the publisher reads `/v1/models` and the 402 arrives on `/chat/completions` |
| `reachableIsNotAddressable` *(re-audit)* | **"The service answers" and "this caller may use it" are different measurements, and the first is the cheap one** | Kaana is reachable from anywhere and Alia still cannot call it: it refuses anything that is not an Ed25519-signed Oxy edge envelope and holds only *public* keys. Stopping at `curl /livez → 200` would have unblocked 96 rows onto a door Alia has no key to |
...
97 of the 246 rows. `OxyHQ/Kaana` PR #6 — *"containerize Kaana and add the AWS deploy workflow"* —
merged on 2026-08-18, and Kaana is running. **When the Oxy inference control plane went live is not
...
### What Kaana actually serves, which is less than the epic assumes

**One provider.** `KAANA_PROVIDERS=cerebras`, one base URL, one key. The published inventory
(`s3://oxy-kaana-inventory-usw2-237343248947/inventory/current.json`, `snap_011361d3b1a380da`,
...
`packages/api/src/internal/providers/lib/providers/` against `cmd/kaana/main.go:316-337`, which
...
- **Needs a Go adapter** (neither of Kaana's two protocols): **google**
...
### Alia cannot call Kaana directly, and this is the trap the re-audit nearly fell into
...
| box | the named gap |
| --- | --- |
| L335 *OpenAI and OpenAI voice/realtime* | the **text** half is config-only; **voice/realtime** has no Kaana path — `adapter.go:133` refuses `modality != text` |
...
| L691 *Voice/audio path* (e2e) | same refusal. Alia's voice path is LiveKit and stays outside Kaana |
...
| L368 *Same-model fallback* | built and **withheld**: contract 1.1.0 sends `{routingPolicyId, policyVersion}` and no snapshot, so `routingFallbackPolicy` never reaches the data plane. Kaana ships failover off rather than override a control the platform advertises |
...
**It cannot be flipped today, and Kaana is not why.** The ten variables the boot check demands were
...
`ALIA_KAANA_CLIENT_ENABLED`, `ALIA_KAANA_ACCOUNT_ID`, `ALIA_KAANA_APPLICATION_ID`,
`ALIA_KAANA_CREDENTIAL_ID`, `ALIA_KAANA_CREDENTIAL_KEY`, `ALIA_KAANA_CREDENTIAL_SECRET`,
`ALIA_KAANA_ENVIRONMENT`, `ALIA_KAANA_INFERENCE_SCOPES`, `KAANA_BASE_URL`, `OXY_API_URL`.
...
of it.** The Oxy → Kaana hop reported live above reached production by hand.
...
and `ALIA_KAANA_CLIENT_ENABLED` is the arming switch, which goes **last and alone** — both guards
...
4. **A priced catalogue route that resolves to a Kaana deployment.** The edge refuses
...
5. **A Cerebras account that can be billed.** Kaana serves only Cerebras, and its own
...
branch, and `GET https://api.alia.onl/v1/models` returns an empty list. Kaana is the path that fixes
...
the 2026-08-19 re-audit, and one of those holds in `OxyHQ/Kaana` rather than here**, so its mutation
is marked PROPOSED: applying it would edit another repository's working tree, which this audit did
not do. The *unmutated* half was run — `go test -count=1 ./internal/provider/...` at `OxyHQ/Kaana`
...
**L377 *"Kaana adapter passes conformance tests"* was first classified `ALREADY_TRUE`** on the
strength of that green run, and is `ACTIONABLE_UPSTREAM` instead. Issue #139 line 375 reads **"For
each provider:"** three lines above it, so the gate is per-provider and Kaana ships adapters for
...
| --- | --- | --- |
| **L338 Cerebras** *(re-audit)* | **partial, and weaker than it looks.** `internal/provider/openaicompat` serves it and *is* guarded by Kaana CI's conformance run; but the `knownProviders` entry (`cmd/kaana/main.go:337`) is referenced by **no** `_test.go`, and `KAANA_PROVIDERS=cerebras` on `oxy-kaana:2` is deployment config no repository asserts | **PROPOSED.** Delete the `"cerebras"` entry from `knownProviders` → startup fails `KAANA_PROVIDER_CEREBRAS_PROTOCOL is required` (`main.go:412`). **Note what this does not cover:** drop `cerebras` from `KAANA_PROVIDERS` instead and the box becomes false with nothing going red anywhere |
| **L540 Pin allowed Kaana origins** *(ticked earlier)* | `kaana-endpoint.ts` `KAANA_ALLOWED_ORIGINS`, fail-closed through `kaanaBootConfigurationFailure` | unchanged. The re-audit adds only the measurement that one of the two pinned origins is currently unusable by Alia |
| L212 tools/structured output/vision/… **(ticked since)** | `kaana-request.ts:340` (moved by #164), called by `kaana-client.ts:931` | replacing the tools condition with `false` failed `kaana-request.test.ts:357` — measured, 1 failed / 25 passed |
...
| L522 no provider API key in a deployment environment | **no guard.** Holds by absence — see premise (d). #164's `direct-provider-guard.ts` names provider credentials but is armed only by `ALIA_KAANA_CLIENT_ENABLED`, which no deployment sets | none. Adding `process.env.OPENAI_API_KEY` to any source file still fails nothing |
...
`kaana-request.ts:219` failed `kaana-request.test.ts:357` (1 failed, 25 passed). Replacing
...
| --- | ---: | --- | --- | ---: | --- |
| `alia_model_provider_mappings` | 10 | kaana | `0003_closed_black_queen.sql` | 0 | 0 |
| `alia_models` | 10 | alia | `0003_closed_black_queen.sql` | 0 | 0 |
| `api_usage` | 10 | kaana | `0000_mature_marauders.sql` | 0 | 0 |
| `chat_analytics` | 10 | alia | `0001_natural_vulture.sql` | 0 | 0 |
| `cost_entries` | 10 | kaana | `0001_natural_vulture.sql` | 0 | 0 |
| `external_models` | 10 | alia | `0003_closed_black_queen.sql` | 0 | 0 |
| `fallback_events` | 10 | kaana | `0000_mature_marauders.sql` | 0 | 0 |
| `model_configs` | 10 | kaana | `0003_closed_black_queen.sql` | 0 | 0 |
| `provider_health` | 10 | kaana | `0000_mature_marauders.sql` | 0 | 0 |
| `provider_keys` | 10 | kaana | `0003_closed_black_queen.sql` | 0 | 0 |
...
`kaana-error.ts:30`, `kaana-openai-adapter.ts:32`, `kaana-boot-check.ts:54`) and six of their test
...
*Re-audit note (2026-08-19):* it does not wait on Kaana, and it never did — `availabilityScope` is an
...
- ~~**Kaana's own repository, beyond its file tree and README.** Whether its adapters pass their
  conformance suite, and whether anything deploys it, were not run.~~ Both were: the conformance
  suite is green at `254d545`, and `OxyHQ/Kaana` deployed on 2026-08-18.
...
- **Whether the Cerebras account can be billed.** The last measurement anyone recorded is Kaana's
